...
class Parser extends hxparse.Parser<hxparse.LexerTokenSource<Token>, Token> {
	...
	static function isDecimal(ch:Int):Bool {
		return '0'.charCodeAt(0) <= ch && ch <= '9'.charCodeAt(0);
	}

	static function isIdent(ch:Int, pos:Int):Bool {
		return ch == '_'.charCodeAt(0) || 'A'.charCodeAt(0) <= ch && ch <= 'Z'.charCodeAt(0) || 'a'.charCodeAt(0) <= ch && ch <= 'z'.charCodeAt(0) || isDecimal
			(ch) && pos > 0;
	}

	static function isDigit(ch:Int):Bool {
		return '0'.charCodeAt(0) <= ch && ch <= '9'.charCodeAt(0) || 'a'.charCodeAt(0) <= ch && ch <= 'f'.charCodeAt(0) || 'A'.charCodeAt(0) <= ch && ch <= 'F'
			.charCodeAt(0);
	}
																			...
																			case [_ => v]: {
																					switch v {
																						case {tok: PClose} : {
																								stat = STable(ETable([{
																									key: stat,
																									value: EFunctionCall(null, null, method, params, false)
																								}]));
																								switch stream {
																									case [{tok: Col} | {tok: Dot}]: {
																											reRun();
																													...
																													case {tok: PClose} :
																														{
																															stat = STable(ETable([{
																																key: stat,
																																value: EFunctionCall(null,
																																	EIdent(s), null, nParams,
																																	false)
																															}]));
																															switch stream {
																																case [{tok: Col} | {tok: Dot}]:
										...
										case [{tok: Binop(op)}]:
											{
												...
												switch op {
													...
													case OpMult:
														{
															ret.push(EArithmeticOp(id, exp, "x"));
														};
													case OpDiv: {
															ret.push(EArithmeticOp(id, exp, "/"));
														}
									...
									case [{tok: Binop(op)}]:
										{
											...
											switch op {
												...
												case OpMult:
													{
														ret.push(EArithmeticOp(id, rhs, "x"));
													};
												case OpDiv: {
														ret.push(EArithmeticOp(id, rhs, "/"));
													}
